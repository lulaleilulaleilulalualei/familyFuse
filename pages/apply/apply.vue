<template>
	<view class="apply">
		<cu-custom :isShowBorder="isShowNavBorder" :bgColor="navColor" :isBack="!isShareStatus">
			<block slot="content"><text :style="{'opacity': navTitle}">活动报名</text></block>
		</cu-custom>
		<view class="contentBox">
			<scroll-view class="contentBoxScroll" :scroll-y="!isShowFeeBox && !isShowOthInit" @scroll="toScroll" :scroll-into-view="viewId" :scroll-with-animation="true">
				<view class="joinBanner">
					<image lazy-load="true" :src="dataObj.banner" mode="aspectFill"></image>
					<view class="show-init-list">
						<scroll-view class="show-init-list-scroll" :scroll-y="true" >
							<view class="show-init-list-box">
								<view class="show-init-icon"><image src="/static/name_init.png" mode=""></image></view>
								<view class="to-show-init-info-box" >
									<view class="item-show-init-info-box">
										<view class="show-init-head">
											<image :src="dataObj.avatar || '/static/oth_icon.png'" mode="aspectFill"></image>
										</view>
										<view class="show-init-info-box">
											<view class="show-init-name">
												<text>{{dataObj.creator || '--'}}</text>
											</view>
											<view class="show-init-tel">{{dataObj.tel || '--'}}</view>
										</view>
									</view>
								</view>
								<template v-if="unionArr.length != 0">
									<view class="to-show-init-info-box" v-for="(item, idx) in unionArr" :key="idx">
										<view class="item-show-init-info-box">
											<view class="show-init-head"><image lazy-load="true" :src="item.union_img || '/static/oth_icon.png'" mode="aspectFill"></image></view>
											<view class="show-init-info-box">
												<view class="show-init-name">
													<text>{{item.union_name || '--'}}</text>
												</view>
												<view class="show-init-tel">{{item.union_tel || '--'}}</view>
											</view>
										</view>
									</view>
								</template>
					...
					<view class="to-show-init-info"></view>
					<view class="to-opt-init-btn">
						<view class="to-edit-oth-init" v-if="dataObj.uid == userInfo.uid" @tap="toEditJoinInit"><text class="cuIcon-edit lg"></text></view>
						<view class="to-show-oth-init-btn" v-if="unionArr.length != 0 && unionArr.length > 5" @tap="toShowMoreInit"></view>
					</view>
				</view>
				<view class="joinBox">
					...
					<view class="cur-show-spot">
						<swiper class="cur-swiper-box" :autoplay="true" :interval="5000" :duration="800" @change="curSwiperChange">
					...
					<view class="had-apply-box">
						<view class="had-apply-title">已报名</view>
						<view class="to-show-name-list" :style="{height: isShowNameList? 'auto': '360rpx'}">
							<view class="item-name-box" v-for="(item, idx) in dataObj.apply_list" :key="idx">
								<view class="item-name-img">
									<view class="item-name-img-box">
										<image lazy-load="true" :src="item.avatar || '/static/oth_icon.png'" mode="aspectFill"></image>
									</view>
									<view class="item-oth-dear-dot" v-if="item.is_own_dear == 0"></view>
								</view>
								<view class="item-name-txt">{{item.name}}</view>
							</view>
							<view class="no-data-box" v-if="dataObj.apply_list.length == 0">
								<view class="noDataImg" style="width: 100px;height: 110px"><img src="/static/noData.png" alt=""></view>
								<view class="noDataText">还没有人报名~</view>
							</view>
						</view>
						<view class="to-opt-apply-list" @tap="toOptApplyList" v-if="dataObj.apply_list.length > 8">
							<view :class="['to-opt-apply-txt', {'to-active-apply-list': isShowNameList}]">{{optApplyTxt}}</view>
						</view>
					</view>
				</view>
			</scroll-view>
			...
			<view class="apply-buttom-btn">
				<view class="to-home" v-if="isShareStatus || isFromStartName" @tap="toBackHome">
					<view class="to-home-icon"><image src="/static/to_home_icon.png" mode=""></image></view>
					<view class="to-home-txt">首页</view>
				</view>
				<view class="to-init-funding" @tap="toStartCF">发起助力</view>
				<view class="to-apply-btn" @tap="toApplyActivity">我要报名</view>
		...
		<view class="to-apply-box" v-if="isShowFeeBox" data-sign="out" @click="toCloseFeeBox">
			<view class="apply-fee-box">
				...
				<view class="item-apply-fee">
					...
					<view class="item-fee-num">¥{{dataObj.pay_fee}}</view>
				</view>
				<view class="item-apply-fee">
					...
					<view class="item-fee-num">¥{{dataObj.service_fee}}</view>
				</view>
		...
		<!-- 查看发起人信息 -->
		<view class="to-init-info" v-if="isShowOthInit" data-sign="out" @click="toCloseInitBox">
			<view class="init-info-box">
				<scroll-view :scroll-y="true" >
					<view class="init-info-title">姓氏发起人</view>
					<view class="item-init-info">
						<view class="item-init-head"><image :src="dataObj.avatar || '/static/oth_icon.png'" mode="aspectFill"></image></view>
						<view class="item-init-info-box">
							<view class="item-init-info-name">{{dataObj.creator}}</view>
							<view class="item-init-info-tel">{{dataObj.tel}}</view>
						</view>
					</view>
					<view class="item-init-info" v-for="(item, idx) in unionArr" :key="idx">
						<view class="item-init-head"><image lazy-load="true" :src="item.union_img || '/static/oth_icon.png'" mode="aspectFill"></image></view>
						<view class="item-init-info-box">
							<view class="item-init-info-name">{{item.union_name}}</view>
							<view class="item-init-info-tel">{{item.union_tel}}</view>
						</view>
					</view>
				</scroll-view>
			</view>
		...
		<showPosterBox :_isShowBox="isShowPorsterBox" :dear_id="dear_id" @toClosePosterBox="toClosePosterBox"></showPosterBox>
	...
	export default {
		data() {
			return {
				...
				isAuth: false, //登录状态
				isShareStatus: false,
				isFromStartName: false, //是否来自添加发起人
			}
		},
		components: { showPosterBox, loadErr },
		onLoad(opt) {
			 ...
			 if(opt.from  && opt.from == 'startName') {
			 	this.isFromStartName = true
			 }
		...
		methods: {
			...
			toEditJoinInit() {
				const $this = this
				if(this.dataObj.uid == this.userInfo.uid) {
					uni.navigateTo({
						url: `/pages/initName/initName?from=edit&id=${$this.dear_id}`
					})
				}
			},
			...
			toShowMoreInit() { //查看联合发起人
				if(this.unionArr.length != 0) {
					this.isShowOthInit = true
				}else {
					this.$api.warnNotice('您尚未添加联合发起人')
				}
			},
			toCloseInitBox(e) { //关闭
				let sign = e.target.dataset.sign
				if(sign) {
					this.isShowOthInit = false
				}
			},
			toBackHome() {
				uni.reLaunch({
			...
			getData() {
				...
				this.$api.ajax({
					url: this.$config.applyActDetail,
					data: {
						no: this.$config.no,
				...
				.then((res) => {
					if(res.statusCode == 200 && res.data.code == 0) {
						...
						obj.tel = obj.tel.substr(0, 3) + '********'
						
						this.dataObj = obj
						this.lineObj = JSON.parse(obj.content)
						this.unionArr = JSON.parse(obj.union_json) || []
						if(this.unionArr.length != 0) {
							this.unionArr.map((v, i) => {
								let stratName = v.union_name.split(' | ')[1]
								if(stratName == '会长' || stratName == '执行会长') {
									v.union_tel = v.union_tel.substr(0, 3) + '********'
								}
							})
						}
			...
			toStartCF() { //发起众筹
				const $this = this
				this.toGetActInfo(this.dataObj)
				uni.navigateTo({
					url: `/pages/writeOrder/writeOrder?type=2&dear_id=${$this.dear_id}`
				})
			},
			toJumpApply() {
				const $this = this
				this.toGetActInfo(this.dataObj)
				uni.navigateTo({
					url: `/pages/writeOrder/writeOrder?type=1&dear_id=${$this.dear_id}`
				})
			},
			onShareAppMessage() {
				const $this = this
...
<style scoped lang="scss">
	...
	.joinBanner {
		...
		.show-init-list {
			...
			.to-show-init-info-box {
				overflow: hidden;
				display: flex;
				&:not(:last-child) {
					margin-bottom: 20upx;
				}
				.item-show-init-info-box {
					display: flex;
					align-items: center;
					padding: 10upx 20upx;
					border-radius: 20upx;
